...
### Compatibility
It has been verified to work in the default GCC to USB adapter configuration, as well as in the Joybus configuration, with a Mayflash adapter. If you have additional hardware you would like to verify, please do! And if you have thoughts on DTA implementations, please let me know, or feel free to mess aroud with the code. The changes are exclusive to the *analog_press.cpp* file. All other files besides *main.cpp* are unchanged. In main, you can change the DacAlgorithms to AnalogPress to implement the control scheme.
