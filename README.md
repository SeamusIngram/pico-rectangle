...
### Layout
One thing to note for layouts is that **modifier keys and directional inputs should not be pressed with the same fingers**. There are situations where you will want to be pressing a direction along with at leat one modifier. Therefore, it makes sense to put them on separate fingers. This is relevant because a common layout has medium shield and analog up both pressed by your right pinky, which is not ideal for this scheme. You might need to adjust your layout to find something more comfortable. Also note that you might want to hold **slow** and **hold** together, as well as **notch** and hold**, so it's best not to have them pressed by the same finger, which is why the layout places **notch** and *slow** by your left thumb. It's not ideal to switch which modifier goes where (ie. don't swap your ModX and Medium shield buttons).
### Compatibility
